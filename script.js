...
const paragraphInputSection = document.getElementById(
  "paragraph-input-section"
);
const paragraphDisplaySection = document.getElementById(
...
async function loadModel() {
  try {
    ...
    addMessage(
      "Sorry, there was an error loading the QnA model. Please refresh and try again.",
      "ai"
    );
...
submitParagraphBtn.addEventListener("click", () => {
  ...
  if (paragraph.length < 20) {
    showNotification(
      "Please enter a longer paragraph (at least 20 characters)."
    );
    return;
  }
  ...
  addMessage(
    "Hi there! Ask me questions about the paragraph you provided.",
    "ai"
  );
});
  ...
  try {
    ...
    if (answers && answers.length > 0) {
      ...
      if (bestAnswer.score > 0.01) {
        ...
        addMessage(
          "I couldn't find a specific answer to that question in the paragraph. Could you try rephrasing?",
          "ai"
        );
      }
    } else {
      addMessage(
        "I couldn't find an answer to that in the paragraph. Try asking something else?",
        "ai"
      );
    }
  } catch (error) {
